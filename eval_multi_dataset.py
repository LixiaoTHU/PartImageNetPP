...
from model import build_model, build_part_model
from IN_C_helper import probabilities_to_decision
name2label = {'airplane':0, 'bear':1, 'bicycle':2, 'bird':3, 'boat':4, 'bottle':5, 'car':6, 'cat':7, 'chair':8, 'clock':9, 'dog':10, 'elephant':11, 'keyboard':12, 'knife':13, 'oven':14, 'truck':15}
...
def get_args_parser():
    ...
    parser.add_argument('--data-dir', default="/home/user/ssd2/datasets/", type=str, help='test evluation dir')
    ...
    # data parameters
    parser.add_argument('--dataset', default='IN-C', type=str, help='dataset for evaluation',choices=['IN-C','IN-A-Plus','SIN','IN-R','IN-Sketch','cue-conflict'])
...
def main(args):
    ...
    # model = torch.nn.Sequential(OrderedDict([("normalization", normalize), ("model", model)]))
    ...
    if args.dataset == 'IN-C':
        with open(os.path.join(args.output_dir,'IN-C-results.txt'),'w+',encoding='utf-8') as f:
            final_acc = 0
            for corruption in ['gaussian_noise', 'shot_noise', 'impulse_noise', 'defocus_blur',
                        'glass_blur', 'motion_blur', 'zoom_blur', 'snow', 'frost', 'fog',
                        'brightness', 'contrast', 'elastic_transform', 'pixelate', 'jpeg_compression']:
                        # 'speckle_noise', 'gaussian_blur', 'spatter','saturate']:
                average = 0
                for severity in range(1,6):
                    inc = datasets.ImageFolder(root=os.path.join(args.data_dir,"IN-C",corruption,str(severity)), transform=test_transform)
                    dataloader_eval = torch.utils.data.DataLoader(inc, 
                    ...
                    clean_acc=validate(model, dataloader_eval, args, classmask=class_mask)
                    average += clean_acc['top1']
                average = average / 5
                print('*******Final Top1 acc of clean images of '+corruption+' is: {0:>7.4f}'.format(average))
                f.write('Final Top1 acc of clean images of '+corruption+' is:'+str(average)+'\n')
                final_acc += average
            final_acc = final_acc / 15
            print('*******Final Top1 acc of clean images is: {0:>7.4f}'.format(final_acc))
            f.write('Final Top1 acc of clean images is:'+str(final_acc)+'\n')
        return
    elif args.dataset == 'IN-R':
        ...
        class_mask = [wnid in imagenet_r_wnids for wnid in all_wnids]
        imagenet_r = datasets.ImageFolder(root=os.path.join(args.data_dir,"IN-R"), transform=test_transform)
        dataloader_eval = torch.utils.data.DataLoader(imagenet_r, 
                                                    batch_size=args.batch_size * n_gpu * 2,
                                                    shuffle=True,
                                                    num_workers=args.num_workers,
                                                    sampler=None,
        ...
        if args.per_class_acc:
            classes = sorted(os.listdir(os.path.join(args.data_dir,"IN-R")))
            
    elif args.dataset == 'IN-A-Plus':
        args.num_classes = 200
        ...
        class_mask = [k for k in thousand_k_to_200 if thousand_k_to_200[k] != -1]
        aplus_transform = transforms.Compose([transforms.Resize((256,256)), transforms.CenterCrop((224,224)), transforms.ToTensor()])
        naes = datasets.ImageFolder(root=os.path.join(args.data_dir,"IN-A-Plus"), transform=aplus_transform)
        dataloader_eval = torch.utils.data.DataLoader(naes, 
                                                batch_size=args.batch_size * n_gpu * 2,
                                                shuffle=True,
                                                num_workers=args.num_workers,
                                                sampler=None,
        ...
        if args.per_class_acc:
            classes = sorted(os.listdir(os.path.join(args.data_dir,"IN-A-Plus")))
    
    elif args.dataset == 'IN-Sketch':
        imagenet_sketch = datasets.ImageFolder(root=os.path.join(args.data_dir,"IN-Sketch"), transform=test_transform)
        dataloader_eval = torch.utils.data.DataLoader(imagenet_sketch, 
                                                    batch_size=args.batch_size * n_gpu * 2,
                                                    shuffle=True,
                                                    num_workers=args.num_workers,
                                                    sampler=None,
        ...
        stylized = datasets.ImageFolder(root=os.path.join(args.data_dir,"SIN"), transform=test_transform)
        dataloader_eval = torch.utils.data.DataLoader(stylized, 
                                                    batch_size=args.batch_size * n_gpu * 2,
                                                    shuffle=True,
                                                    num_workers=args.num_workers,
                                                    sampler=None,
...
def validate(model, loader, args, classmask=None, log_suffix='clean acc'):
