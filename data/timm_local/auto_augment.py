...
class RandAugment:
    def __init__(self, ops, num_layers=2, choice_weights=None):
        ...
        self.aligned_transform = ["Rotate", "ShearX", "ShearY", "TranslateXRel", "TranslateYRel"]
    ...
    def __call__(self, img, mask = None):
        ...
        for op in ops:
            ...
            img = op(img)
            if mask is not None and op.name in self.aligned_transform:
                if isinstance(mask, list):
                    newmask = []
                    for m in mask:
                        set_random_states(state)
                        fillcolor = copy.copy(op.kwargs["fillcolor"])
                        resample = copy.copy(op.kwargs["resample"])
                        op.kwargs["fillcolor"] = 0
                        op.kwargs["resample"] = Image.NEAREST
                        m = op(m)
                        op.kwargs["fillcolor"] = fillcolor
                        op.kwargs["resample"] = resample
                        newmask.append(m)
                    mask = newmask
                else:
                    set_random_states(state)
