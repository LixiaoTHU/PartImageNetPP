...
def adv_generator(args, images, target, model, eps, attack_steps, attack_lr, random_start, attack_criterion='regular', use_best=True, scaler=None, _logger = None):
    ...
    for _ in range(attack_steps):
        ...
        with autocast(enabled = scaler is not None):
            ...
            # backward
            adv_losses = torch.mean(adv_losses)

        if scaler is not None:
            scaler.scale(adv_losses).backward()
        else:
            adv_losses.backward()
        ...
        if torch.isnan(grad).any() or torch.isinf(grad).any():
            _logger.info('%s: nan or inf in adv grad!' % (str(args.rank)))
    ...
    if use_best:
        ...
        best_loss, best_x = adv_losses, images
    if prev_training:
        model.train()
