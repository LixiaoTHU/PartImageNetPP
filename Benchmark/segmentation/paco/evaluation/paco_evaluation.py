...
def _evaluate_predictions_on_lvis(
    ...
    # 获取所有的 category IDs
    #cat_ids = [x["id"] for x in lvis_eval.lvis_gt.dataset["categories"]]

    obj_cat_ids = [x["id"] for x in lvis_eval.lvis_gt.dataset["categories"] if x["supercategory"] == "object"]
    part_cat_ids = [x["id"] for x in lvis_eval.lvis_gt.dataset["categories"] if x["supercategory"] == "part"]
    obj_cat_ids.sort()
    part_cat_ids.sort()
    # print(len(obj_cat_ids))

    # print(len(part_cat_ids))

    """
    # 按照 ID 从小到大排序
    cat_ids.sort()

    # 取后 1000 个作为 object_ids，剩下的为 part_ids
    obj_cat_ids = cat_ids[-1000:]
    part_cat_ids = cat_ids[:-1000]
    print(len(obj_cat_ids))
    ...
    #for obj_cat in obj_cat_ids:
        ...
        #obj_results_per_class[all_obj_names[cat_ids]] = ap * 100

    results_processed["obj-AP"] = get_mean_AP(obj_results)
    ...
    for idx in part_cat_ids:
        ap = get_AP_from_precisions(precisions, idx)
        part_results.append(float(ap * 100))

    results_processed["part-AP"] = get_mean_AP(part_results)
    ...
    # print(results['APs'], 'asd')
    # print(results['APm'], 'asd')
    # print(results['APl'], 'asd')
