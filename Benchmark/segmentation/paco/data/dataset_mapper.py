    ...
    if "segmentation" in annotation:
        ...
        if isinstance(segm, list):
            ...
            try:
                ...
                print(f"Error processing polygons for annotation: {annotation}")
...
class PACODatasetMapper(DatasetMapper):
    ...
    def _transform_annotations(self, dataset_dict, transforms, image_shape):
        ...
        if len(annos) and "attr_label_tensor" in annos[0]:
            #使用numpy.array()将列表中的多个NumPy数组合并为一个NumPy数组
            attr_label_array = np.array([obj["attr_label_tensor"] for obj in annos])
            attr_ignore_array = np.array([obj["attr_ignore_tensor"] for obj in annos])
            
            # 然后将NumPy数组转换为PyTorch张量
            attr_label_tensor = torch.tensor(attr_label_array)
            attr_ignore_tensor = torch.tensor(attr_ignore_array, dtype=torch.int64)
